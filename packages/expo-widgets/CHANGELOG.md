...
- [iOS] Fix the `Build ExpoWidgets Bundle` script phase failing — or silently skipping the bundle — when the project path contains a space, by quoting the script path that `bash -l -c` re-parses in `ExpoWidgets.podspec` and the `basename` operand in `xcode-build-bundle.sh`. ([#48705](https://github.com/expo/expo/issues/48705))
