...
17. **A decorative option set.** A fix-mode `pr.md` should carry a marker-wrapped "Options considered" block; treat its entries as claims like any other. Attack: a straw option nobody would pick, written to make the chosen one look inevitable; options that address different failure mechanisms (a sign the cause is not established); a cost that is not checkable ("less clean", "more complex"); and — MUST-FIX — any wording that presents a non-chosen option as built or tested when the log shows only the chosen one was. The reverse failure matters equally: a single-option set with a stated reason is a valid record, and demanding alternatives for a change with no design space is what produces straw — do not. A missing block on a fix-mode draft is SHOULD-FIX: the agent can add the honest record without new evidence.
18. **Prose that breaks Simplified Technical English.** The draft's prose must follow the ASD-STE100 rules its prompt states: one term per thing, sentences of 20 words or fewer, active voice, plain words, no idiom or hedge. Grade violations SHOULD-FIX — they cost non-native readers time, and the agent can rewrite without new evidence. Quoted code, commands and their output, error strings, identifiers, and file paths are verbatim and exempt; do not flag them.
